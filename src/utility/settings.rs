...
pub fn init() -> std::io::Result<()> {
    ...
    if let Some(base_dirs) = BaseDirs::new () {
        ...
        let bog_path = format!("{}{}", home_dir.to_str().unwrap(), "/Desktop/Code/Rust/BOG_Test/.bog/");
        let path = format!("{}{}", home_dir.to_str().unwrap(), "/Desktop/Code/Rust/BOG_Test/.bog/.bog_settings");
        ...
        let read_file: Settings = toml::from_str(&file)
            .unwrap_or(
                Settings{current_dir: path.clone(), items: Vec::new()}
            );
...
fn create_test_data(path: &String, ) -> Settings {
    ...
    Settings{current_dir: path.clone(), items: things}
}
